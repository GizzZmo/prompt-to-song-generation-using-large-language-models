...
class Single_Core_Trainer:
    ...
    def training_loop(
        ...
        epochs: int,
        optim_alog: torch.optim,
        learning_rate_scheduler: torch.optim = None,
    ) -> dict:
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if (epoch + 1 + pretrained_num_epochs) % 1 == 0:
                print(
                    "For Epoch {} We Train Loss:{}, Val Loss:{}, Train Accuracy:{}, Val Accuracy:{}".format(
                        epoch + 1 + pretrained_num_epochs, epoch_loss, loss, epoch_accuracy, accuracy
                    )
                )
            ...
            if (epoch + 1 + pretrained_num_epochs) % 1 == 0:
                torch.save(
                    model.state_dict(),
                    os.path.join(checkpoint_save_dir, "epoch_" + str(epoch + 1 + pretrained_num_epochs) + ".pth"),
                )
...
class Mixed_Precision_Single_Core_Trainer(Single_Core_Trainer):
    ...
    def _one_epoch_train(
        self: "Mixed_Precision_Single_Core_Trainer",
        model: torch.nn.Module,
        data_loader_train: torch.utils.data.DataLoader,
        optim_alog: torch.optim,
    ) -> tuple:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transformer_name", required=True, metavar="<'bert-base-uncased'>", help="Hugging Face Transformer Name", type=str
    )

    parser.add_argument(
        "--tokenizer_name", required=True, metavar="<'bert-base-uncased'>", help="Hugging Face Tokenizer Name", type=str
    )
    ...
    parser.add_argument(
        "--batch_size", required=False, default=128, metavar="<batchsize to train>", help="Batchsize to Train", type=int
    )

    parser.add_argument("--learning_rate", required=False, default=3e-4, metavar="<float>", help="Learning Rate", type=float)
    ...
    parser.add_argument("--epochs", required=True, metavar="<integer>", help="Number of Epoches", type=int)

    parser.add_argument(
        "--checkpoint_epoches", required=False, default=0, metavar="<integer>", help="What is the checkpoint epoch?", type=int
    )

    parser.add_argument(
        "--checkpoint_load_path", required=False, default=None, metavar="/path/to/model.pt/", help="Path to model.pt file"
    )
    ...
    writer = SummaryWriter(
        ...
        + transformer_name.replace("/", "_")
        + "_freeze_"
        + str(freeze_transformer)
        + "_lr_"
        + str(learning_rate).replace(".", "p")
        + "_mpt_"
        + str(mixed_precision_training)
    )
